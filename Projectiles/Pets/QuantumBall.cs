...
namespace ExtraPets.Projectiles.Pets
{
	public class QuantumBall : ModProjectile
	{
		...
		const int fadeInTicks = 1;
		const int fullBrightTicks = 200;
		const int fadeOutTicks = 1;
		const int range = 500;
		...
		public override void AI()
		{
			...
			if (player.dead)
			{
				modPlayer.quantumBall = false;
			}
			if (modPlayer.quantumBall)
			{
